...
def run_dtcontrol(base_dir):
    command = f"python3 -u run_dtcontrol_iterative_vanilla.py {base_dir}"
    run_command(command)


def postprocess(base_dir):
    command = f"python3 -u postprocess_vanilla.py {base_dir}"
    run_command(command)


def create_table(base_dir):
    command = f"python3 -u create_table_vanilla_iterative.py {base_dir}"
    run_command(command)
